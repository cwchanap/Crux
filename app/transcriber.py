...
class DrumTranscriber:
    ...
    def _build_egmd_architecture(self):
        ...
        velocity_outputs = tf.keras.layers.Dense(9, activation='linear', name='velocity')(velocity_x)
        
        model = tf.keras.Model(
            inputs=inputs,
            outputs={'onset_probs': onset_outputs, 'frame_probs': frame_outputs, 'velocity': velocity_outputs}
        )
    ...
    def _predictions_to_events(self, predictions: Dict[str, np.ndarray]) -> Dict[int, list]:
        ...
        for drum_idx in range(onset_probs.shape[1]):
            ...
            onset_frames = self._find_onset_frames(
                onset_probs[:, drum_idx],
                frame_probs[:, drum_idx],
                onset_threshold
            )
