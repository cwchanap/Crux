...
class ConvStack(tf.keras.Model):
    ...
    def __init__(self, hparams):
        ...
        for i, (conv_temporal_size, conv_freq_size, n_filters, 
                freq_pool_size, dropout_amt) in enumerate(
                    zip(temporal_sizes, freq_sizes, num_filters, 
                        pool_sizes, dropout_keep_amts)):
    ...
    def call(self, inputs, training=False):
        ...
        for i, (conv, bn, pool, dropout) in enumerate(
                zip(self.conv_layers, self.batch_norms, 
        ...
        for i in range(stack_size):
            self.lstm_layers.append(
                layers.Bidirectional(
                    layers.LSTM(num_units, return_sequences=True),
                    name=f'bilstm_{i}'
                )
            )
...
class OnsetsFramesModel(tf.keras.Model):
    ...
    def __init__(self, hparams=None):
        ...
        self.onset_model = AcousticModel(
            ...
            hparams.acoustic_rnn_stack_size
        )
        ...
        self.velocity_model = AcousticModel(
            hparams,
            hparams.velocity_lstm_units,
            hparams.acoustic_rnn_stack_size
        )
        
        self.frame_model = AcousticModel(
            hparams,
            hparams.frame_lstm_units,
            hparams.acoustic_rnn_stack_size
        )
        ...
        if hparams.combined_lstm_units > 0:
            self.combined_lstm = BiLSTMLayer(
                ...
                hparams.combined_rnn_stack_size
            )
        else:
