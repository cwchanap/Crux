...
4. It excludes `__pycache__` entries and `*.pyc` from identity and from the
   ...
   environment drift.
5. It descriptor-walks the exclusive roots (`purelib`/`platlib`, the
   distribution installation trees) and rejects every other non-bytecode
   regular file there. Thus injected `.py`, `sitecustomize.py`, and `.pth`
   files must be declared by a distribution or fail attestation. Shared
   roots (`scripts`, `data`, `stdlib`, `include`, ...) are not enumerated:
   only the expected distribution-owned members are opened and checked for
   regular-file type and inventory identity, so undeclared files in shared
   roots are not rejected.
