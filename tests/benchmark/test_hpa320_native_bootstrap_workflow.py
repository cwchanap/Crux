...
WORKFLOW = Path(".github/workflows/hpa320-native-bootstrap.yml")
...
def test_bootstrap_workflow_is_exact_commit_native_linux_x64() -> None:
    text = _load()
    ...
    assert '[[ "${COMMIT_SHA}" =~ ^[0-9a-f]{40}$ ]]' in text
    assert "observe-github" in text
    assert "finalize-github" in text
    assert "--phase bootstrap" in text


def test_bootstrap_workflow_pins_tools_and_only_publishes_review_artifacts() -> None:
    text = _load()
    assert '\nenv:\n  UV_FROZEN: "1"\n\njobs:\n' in text
    ...
    assert "bootstrap-image" in text
    assert "attest-base-system" in text
