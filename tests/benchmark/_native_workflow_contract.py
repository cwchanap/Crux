...
def _load_text(workflow_path: Path) -> str:
    return workflow_path.read_text(encoding="utf-8")


def _load(workflow_path: Path) -> dict[str, object]:
    workflow = yaml.safe_load(_load_text(workflow_path))
...
def _assert_native_workflow_contract(
    ...
    *,
    ...
    serialized = _load_text(workflow_path)
    for forbidden in (
        "observe-native-host",
        "observe-github",
        "finalize-github",
    ...
    assert publish["run"] == (
        ...
        f"{payload_root}/{phase}-host-attestation/attestation-bundle.json --archive {archive_run}"
    )
    ...
    assert verify_before_attestation["run"] == (
        ...
        f"--phase {phase} --payload-root {payload_root} --archive {archive_run}"
    )
    ...
    assert f"--destination {bundle_run}" in copy_bundle_run
    assert f"tee {identity_run}" in copy_bundle_run
    reverify = _step_named(steps, "Reverify the complete upload set")
    assert reverify["run"] == (
        "uv run python -m tools.hpa320.oaf_native_artifacts verify "
        f"--phase {phase} --payload-root {payload_root} --archive {archive_run} "
        f"--bundle {bundle_run} --expected-bundle-identity {identity_run}"
    )
