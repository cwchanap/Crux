...
WORKFLOW = Path(".github/workflows/hpa320-native-candidate.yml")
...
def test_candidate_workflow_is_exact_commit_native_linux_x64() -> None:
    text = _load()
...
def test_candidate_workflow_calibrates_without_publishing_final_locks() -> None:
    text = _load()
    assert '\nenv:\n  UV_FROZEN: "1"\n\njobs:\n' in text
    ...
    assert text.index("bootstrap-image") < text.index("seal_oaf_backend calibrate")
    assert "seal-profile-request.json" in text
    assert "calibration-measurement-evidence.json" in text
